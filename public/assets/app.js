...
async function renderMessages({ smoothScroll = false, scroll = false, extraSpace = 100 } = {}) {
  try {
    const res = await fetch('/api'); // Node API
    ...
    if (scroll) {
      // Scroll window to bottom + extraSpace
      requestAnimationFrame(() => {
        window.scrollTo({
          top: document.body.scrollHeight + extraSpace,
...
async function addMessage() {
  ...
  await renderMessages({ smoothScroll: true, scroll: true, extraSpace: 100 });
}

window.addEventListener('DOMContentLoaded', () => {
  document.getElementById('addMessageBtn')?.addEventListener('click', addMessage);

  // Initial load: scroll to last message + extra space
  renderMessages({ smoothScroll: true, scroll: true, extraSpace: 100 });

  // Auto-refresh every 5s, but don't scroll
  setInterval(() => renderMessages({ scroll: false }), 5000);
});
